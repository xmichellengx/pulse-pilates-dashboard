...
const s = StyleSheet.create({
  ...
  // Same as `page` but with a tighter bottom margin. Applied only when the
  // warranty footer is present, so the document still fits one sheet without
  // changing the layout of every other document type.
  pageCompact: {
    fontFamily: "Helvetica",
    fontSize: 9,
    color: "#1a1a1a",
  ...
  // Layered over the base styles (via style arrays) only when the warranty
  // footer is present, to buy back the ~50pt it needs.
  tableRowCompact: { paddingVertical: 5 },
  importantBoxCompact: { marginTop: 9, paddingTop: 5 },
  importantItemCompact: { marginBottom: 2 },
  bankLineCompact: { marginTop: 5, paddingTop: 3, marginBottom: 3 },
  forPulseCompact: { marginBottom: 4 },
  ...
  // ── B2C warranty footer (QR + pointer to the full T&Cs) ──
  // Sits below the signature block, deliberately understated: the warranty
  // terms themselves are stated in the T&Cs above, so this is just the route
